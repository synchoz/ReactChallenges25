...
export default function ImageSlider() {
    ...
    function moveToNextImage() {
        arr[currImageIndex].isActive = false;
        const imgIndex =  currImageIndex == arr.length - 1 ? 0 : currImageIndex + 1;
        arr[imgIndex].isActive = true;
        setCurrImageIndex(imgIndex);
        
    }
    function moveToPrevImage() {
        arr[currImageIndex].isActive = false;
        const imgIndex =  currImageIndex == 0 ? arr.length - 1 : currImageIndex - 1;
        arr[imgIndex].isActive = true;
        setCurrImageIndex(imgIndex);
    }
    return(
        <div className="mainContainer">
            <div className="imageContainer">
                ...
                <div className="btnContainer">
                    <div className="btn prevBtn"  onClick={moveToPrevImage}>{/* P */}</div>
                    <div className="btn nextBtn" onClick={moveToNextImage}>{/* N */}</div>
                </div>
